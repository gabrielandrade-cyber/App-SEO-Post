...
export function ImageOptimizer() {
  ...
  const currentProvider = allowedProviders.includes(settings.provider)
    ? settings.provider
    : "gemini";
  ...
  const compressToVercelLimits = async (
    blob: Blob,
    file: File,
    maxMb = 3.5,
  ...
  const callVisionServer = async (id: string, prompt: string, isTitle: boolean) => {
    ...
    if (!apiKey) {
      toast.error(
        `A API Key do ${currentProvider} não está configurada! Vá ao Workspace configurar.`,
      );
      return;
    }
    ...
    try {
      ...
      console.error(error);
      const message = error instanceof Error ? error.message : "Falha na comunicação com a IA.";
      toast.error(message);
      setImages((prev) =>
        prev.map((img) =>
      ...
      <div className="mx-auto max-w-5xl space-y-8">
        <div className="text-center space-y-3">
          ...
          <p className="text-sm text-white/50 max-w-xl mx-auto">
            Arraste imagens JPG/PNG para convertê-las instantaneamente para WebP. Utilize a visão da
            IA para gerar <b>Nomes de Ficheiro</b> otimizados e <b>Alt Texts</b> perfeitamente
            descritivos para SEO.
          </p>
        </div>
              ...
              <div className="flex items-center gap-2">
                {[
                  ...
                  const active =
                    ...
                    (settings.provider === "cerebras" && opt.id === "gemini");
                  return (
                    <button
                      key={opt.id}
                      onClick={() =>
                        dispatch({ type: "SET_PROVIDER", payload: opt.id as "gemini" | "groq" })
                      }
                    ...
                    >
                      <div
                        className={`flex h-6 w-6 items-center justify-center rounded-lg bg-gradient-to-br ${opt.color} border border-white/10 shadow-inner`}
                      >
                        <img
                          src={opt.img}
                          alt={opt.label}
              ...
              {currentProvider === "gemini" && (
                <div>
                  <label className="mb-1.5 block text-xs font-medium text-white/70">
                    Chave Gemini (aistudio.google.com)
                  </label>
                  <div className="relative">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        dispatch({ type: "SET_GEMINI_KEY", payload: e.target.value })
                      }
                      placeholder="AIzaSy•••"
                      className="w-full rounded-2xl border border-white/10 bg-white/5 py-2 pl-10 pr-3 text-sm text-white placeholder:text-white/30 outline-none backdrop-blur-xl transition-all duration-300 focus:border-white/30 focus:bg-white/10"
                    />
                  </div>
                </div>
              )}
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      onChange={(e) => dispatch({ type: "SET_GROQ_KEY", payload: e.target.value })}
                      placeholder="gsk_•••"
                      className="w-full rounded-2xl border border-white/10 bg-white/5 py-2 pl-10 pr-3 text-sm text-white placeholder:text-white/30 outline-none backdrop-blur-xl transition-all duration-300 focus:border-white/30 focus:bg-white/10"
                    />
                  </div>
          ...
          <div className="flex flex-col items-center justify-center space-y-4">
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-white/40">
                Suporta JPG, PNG. Conversão automática Client-Side (WebP).
              </p>
            </div>
          </div>
          <input
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          <motion.button
                            ...
                            onClick={() =>
                              callVisionServer(
                                img.id,
                                "Crie um nome curto de ficheiro, separado por hifens, altamente otimizado para SEO baseado nesta imagem. Retorne APENAS o nome sem extensão.",
                                true,
                              )
                            }
                          ...
                          <motion.button
                            ...
                            onClick={() =>
                              callVisionServer(
                                img.id,
                                "Escreva um texto alternativo (alt text) descritivo e otimizado para SEO desta imagem. Máximo 120 caracteres. Apenas o texto.",
                                false,
                              )
                            }
